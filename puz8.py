...
if _name_ == "_main_":
    ...
    if solution_path:
        ...
        move_number = 1  # Initialize the move number

        # Iterate through the solution path except for the last step
        for state, direction, _ in solution_path[:-1]:
            # Print the current state before displaying move number and direction
            for row in state:
        ...
        # Print the last state (goal state)
        last_state, _, _ = solution_path[-1]
